...
echo "==> 4/4  Starting FastAPI on http://localhost:8506 ..."
echo "    (Ctrl+C to stop. Open another terminal and run: curl http://localhost:8506/health )"
...
if [ -x "$VENV_BIN/python.exe" ]; then VENV_PY="$VENV_BIN/python.exe"; else VENV_PY="$VENV_BIN/python"; fi
exec "$VENV_PY" -m uvicorn main:app --reload --host 0.0.0.0 --port 8506
